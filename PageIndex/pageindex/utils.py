...
def ChatGPT_API_with_finish_reason(model, prompt, api_key=CHATGPT_API_KEY, chat_history=None):
    max_retries = 10
    client = openai.OpenAI(base_url=os.getenv("OPENAI_BASE_URL", "https://research.neu.edu.vn/ollama/v1"), api_key=api_key or "neu_ollama", timeout=300.0)
    for i in range(max_retries):
        try:
            ...
            if i < max_retries - 1:
                time.sleep(5)  # Wait 5s before retrying (gateway recovery)
            else:
...
async def ChatGPT_API_async(model, prompt, api_key=CHATGPT_API_KEY):
    ...
    for i in range(max_retries):
        try:
            ...
            logging.error(f"Error: {e}")
            if i < max_retries - 1:
                await asyncio.sleep(5)  # Wait 5s before retrying (gateway recovery)
            else:
                logging.error('Max retries reached for prompt: ' + prompt)
                return "Error"  
...
def llm_completion(model, prompt, return_finish_reason=False, chat_history=None):
...
async def llm_acompletion(model, prompt, chat_history=None):
